...
#         self.isWrite = isWrite
    ...
    def __init__(self, filename, isWrite=False):
        ...
        if isWrite:
            ...
            assert os.path.exists(filename), filename
            # Check if the file starts with a YAML header
            with open(filename, 'r') as f:
                first_line = f.readline()
            if not first_line.startswith('%YAML'):
                # If the header is missing, create a temporary file with header prepended
                ...
                with os.fdopen(temp_fd, 'w') as tmp:
                    ...
                    with open(filename, 'r') as orig:
                        tmp.write(orig.read())
                # Open the temporary file with OpenCV FileStorage
                self.fs = cv2.FileStorage(temp_path, cv2.FILE_STORAGE_READ)
                # Save the temporary file path for later cleanup
